...
class AudioProcessor:
    """Базовый интерфейс аудиопроцессора"""
    def set_volume(self, send_fn, level: float):
        raise NotImplementedError


class CP750AudioProcessor(AudioProcessor):
    """Аудио для Dolby CP750: команды те же, префикс cp750"""
    def set_volume(self, send_fn, level: float):
...
class JSD100AudioProcessor(AudioProcessor):
    ...
    def set_volume(self, send_fn, level: float):
        ...
        # Где <XX> — две цифры (00..55), последний символ всегда '0'
        tenths = int(round(float(level) * 10))
        # ограничение 0..55
        tenths = max(0, min(55, tenths))
        two_digits = f"{tenths:02d}"
        payload = f"jsd100.sys.fader\\09{two_digits}0"
        return self._send_jsd(send_fn, payload)

    def select_input_dig1(self, send_fn):
        # Dig1: jsd100.sys.input_mode\094
        payload = "jsd100.sys.input_mode\\094"
        return self._send_jsd(send_fn, payload)

    def select_input_nonsync(self, send_fn):
        # Nonsync: jsd100.sys.input_mode\098
        payload = "jsd100.sys.input_mode\\098"
        return self._send_jsd(send_fn, payload)
...
class BarcoController:
    ...
    def __init__(self, hall_id, host='192.168.1.100', port=43748, audio_vendor: str = 'cp750'):
        ...
        # Выбор аудиопроцессора
        vendor = (audio_vendor or 'cp750').lower()
        if vendor == 'jsd100':
            self.audio = JSD100AudioProcessor()
        else:
            self.audio = CP750AudioProcessor()
        self.audio_vendor = vendor
    ...
    def light_on(self):
        """Включение света через GPIO (Barco ICMP Protocol 0.8)"""
        # Было: EKOS.Send Text,"$KE,WR,4,1\0D\0A"
        return self.send_command('GPIO.Pulse Up,1,1000')
    
    def light_off(self):
        """Выключение света через GPIO (Barco ICMP Protocol 0.8)"""
        # Было: EKOS.Send Text,"$KE,WR,1,1\0D\0A"
        return self.send_command('GPIO.Pulse Up,2,1000')
    
    def set_volume(self, level):
        """Установка громкости через выбранный аудиопроцессор (0-5.5)"""
        return self.audio.set_volume(self.send_command, level)
    
    def set_input_dig1(self):
        """(JSD100) Переключение входа на Dig1, если поддерживается"""
        if isinstance(self.audio, JSD100AudioProcessor):
            return self.audio.select_input_dig1(self.send_command)
        return False, "Недоступно для текущего аудио-вендора"

    def set_input_nonsync(self):
        """(JSD100) Переключение входа на Nonsync, если поддерживается"""
        if isinstance(self.audio, JSD100AudioProcessor):
            return self.audio.select_input_nonsync(self.send_command)
        return False, "Недоступно для текущего аудио-вендора"
    ...
    def shutdown_session(self):
        ...
        # 4. Включение света
        success, response = self.light_on()
...
def init_controllers():
    ...
    for hall in halls:
        ...
        controllers[hall_id] = BarcoController(
            ...
            host=hall['ip'],
            port=hall['port'],
            audio_vendor=hall.get('audio_vendor', 'cp750')
        )
...
def control_light(hall_id, action):
    ...
    if action == 'on':
        ...
        log_action(admin_name, hall_id, 'LIGHT_ON', 'Включение света (GPIO)')
        emit_log(hall_id, f'Свет ВКЛ: {response}', 'success' if success else 'error')
    elif action == 'off':
        success, response = controller.light_off()
        log_action(admin_name, hall_id, 'LIGHT_OFF', 'Выключение света (GPIO)')
...
def set_volume(hall_id):
    ...
    log_action(admin_name, hall_id, 'VOLUME', f'Уровень: {level} ({controller.audio_vendor})')
...
if __name__ == '__main__':
    ...
    for hall_id, controller in controllers.items():
        print(f"  - {hall_id}: {controller.host}:{controller.port} (audio: {controller.audio_vendor})")
    print()
